...
# Produces the perievent average and summary plots for fluorescence as a function of the stimulus inserted in the drawer
...
def get_windows_from_experiment(experiment, events):
    ...
    def get_windows(data, window_size_s, events):
        F = data['dF/F Excitation after isosbestic subtraction']
        ts = data['times']
        ...
        for event in events:
            ...
            if len(event_item) > 0:
                ...
                if window.shape[0] < n:
                    ...
                    #print('window shape adjusted to', window.shape[0])
...
plt.savefig('FigurePdfs/MainPlots/PerieventAveragesForAllExperiments.pdf', transparent=True)
plt.show()
...
def get_groups(data_list, name_list, condition_names, which_event=1, remove_nans=False):
    ...
    for name in condition_names:
        ...
        f = f[:, np.logical_and(data_list[index]['ts'] >= 0, data_list[index]['ts'] <= 5)]
...
def get_t_tests_from_respective_controls(groups, names, control_names):
    ...
    for control_name in unique_control_names:
        ...
        print('Statistics for', control_name, 'vs.', [names[i] for i in indices])
        ...
        print('ANOVA:', 'F = ', F_statistic, 'p =', pvalue, 'n =', [a.shape[0] for a in y])
        ...
        for j in range(len(indices)):
            group_stats = stats.ttest_ind(groups[indices[j]], groups[control_index], equal_var=False)
            group_pvals.append(group_stats.pvalue)
            group_ts.append(group_stats.statistic)


        group_pvals = statfunctions.apply_bonferroni_correction(group_pvals)
        for k in range(len(group_pvals)):
            if group_pvals[k] <= 0.05:
                print('Significant pair:', names[indices[k]], 'vs.', control_name, 't =', group_ts[k], 'p = ', group_pvals[k])
            else:
                print('NON-Significant pair:', names[indices[k]], 'vs.', control_name, 't =', group_ts[k], 'p = ', group_pvals[k])
...
plt.savefig('FigurePdfs/MainPlots/AverageResponseToDrawerStimulus.pdf', transparent=True)
    ...
    if pvals[i] is not None:
        if pvals[i] < 0.01:
            ...
            plt.text(positions[i], star_height, 'N.S.', fontsize=12, horizontalalignment='center',
                     verticalalignment='center')
plt.ylim(ylim)
plt.savefig('FigurePdfs/MainPlots/AverageResponseToDrawerReentry.pdf', transparent=True)
plt.show()
