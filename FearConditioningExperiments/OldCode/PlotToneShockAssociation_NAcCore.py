...
data_directory_path_day_1 = '../../OriginalData/FearConditioningData/Day1/NAcCore/'
data_directory_path_day_2 = '../../OriginalData/FearConditioningData/Day2/NAcCore/'
...
def plot_example_dataset(data):
    ...
    fill_event_ranges((data['shock'] - t_0)/60.0, 1/60.0, color=prettyplot.colors['red'])
...
plt.savefig('NAc_ToneShockExampleRecording.pdf', transparent=True)
plt.show()
...
def collect_trial_windows(data_list, event, window):
    ...
    for data in data_list:
        ...
        try:
            ...
            print('Exception:', e, 'on event', event)
...
maxima_tone1 = np.mean(tone1_window_list[:, :,np.logical_and(0<=ts, ts <= 4.5)], axis=2)
...
maxima_tone_test = np.mean(tone_test_window_list[:, :, np.logical_and(0 <= ts, ts <= 4.5)], axis=2)

# for i in range(11):
#     plot_example_window_list(ts, tone1_window_list[i], figsize=(10, 2), fillregion=[[0, 5], [5, 6]], fillcolor=[[1, 0.85, 0.85], [1, 0.5, 0.5]])
#     #plt.savefig('NAc_ToneShockPeriEventSingleAnimalExample.pdf', transparent=True)
#     prettyplot.title('singleAnimalExample')
#     plt.show()

plot_window_list_average(ts, tone1_window_list, figsize=(10, 2), fillregion=[[0, 5], [5, 6]], fillcolor=[[1, 0.85, 0.85], [1, 0.5, 0.5]], yrange=[-.15, .2])
plt.savefig('NAc_ToneShockPeriEvent.pdf', transparent=True)
prettyplot.title('conditioning')
plt.show()
...
maxima_tone2 = np.mean(tone2_window_list[:, :, np.logical_and(0 <= ts, ts <= 4.5)], axis=2)

plot_window_list_average(ts, tone2_window_list, figsize=(10, 2), fillregion=[[0, 5], [5, 6]], fillcolor=[[1, 0.85, 0.85], [1, 0.5, 0.5]], yrange=[-.15, .2])
plt.savefig('NAc_ToneShockPeriEventDistractorTone.pdf', transparent=True)
prettyplot.title('Distractor')
plt.show()
#
#
plot_window_list_average(ts, tone_test_window_list[:, [0, 4, 9, 14, 19, 24, 29], :], figsize=(10, 2), fillregion=[[0, 5], [5, 6]], fillcolor=[[1, 0.85, 0.85], [1, 0.5, 0.5]], yrange=[-.15, .2])
plt.savefig('NAc_ToneShockExtinctionTrials11015etc.pdf', transparent=True)
prettyplot.title('Extinction')
plt.show()

# NOTE: Should tone_test here be tone1 for NAcCore data?

data_list_day_2 = get_data(data_directory_path_day_2)
ts_day_2, tone_test_window_list_day_2 = collect_trial_windows(data_list_day_2, 'tone1', window)
tone_test_window_list_day_2 = tone_test_window_list_day_2 - np.mean(tone_test_window_list_day_2[:, :, ts < 0], axis=2, keepdims=True)
maxima_tone_test_day_2 = np.mean(tone_test_window_list_day_2[:, :, np.logical_and(0 <= ts, ts <= 4.5)], axis=2)
plot_window_list_average(ts_day_2, tone_test_window_list_day_2[:, [0, 4, 9, 14, 19, 24, 29], :], figsize=(10, 2), fillregion=[[0, 5], [5, 6]], fillcolor=[[1, 0.85, 0.85], [1, 0.5, 0.5]], yrange=[-.05, .3])
plt.savefig('NAc_ToneShockExtinctionTrials151015etcDay2.pdf', transparent=True)
prettyplot.title('day2')
plt.show()
...
prettyplot.no_box()
plt.ylim([-0.1, 0.06])
plt.savefig('NAc_ToneShockToneEvokedResponseAcrossTrials.pdf', transparent=True)
plt.show()

z = maxima_tone1

maxima_tone1 = maxima_tone1  # There's one mouse that didn't get run on the last day. Exclude it for pairwise t-tests
maxima_tone_test = maxima_tone_test
