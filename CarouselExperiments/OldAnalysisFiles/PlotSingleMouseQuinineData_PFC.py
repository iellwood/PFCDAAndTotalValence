...
data_directory_path = '../../OriginalData/CarouselExperimentsData/Quinine/mPFC/'  # directory for data files
...
if True:
    DataProcessingFunctions.plot_dataset(data[7], xlim=[290, 690])
    plt.savefig('FigurePdfs/PFC_ExampleDataset.pdf', transparent=True)
    plt.show()



# Collects all the windows from all the animals. The metadata tells the animal number and port for everyone window
metadata, window_ts, windows = DataProcessingFunctions.get_all_perievent_window_data(data, [5, 10])
...
if False:
    ...
    np.save('PFC_lick_counts', lick_counts)
    ...
    plt.savefig('FigurePdfs/PFC_NumberOfLicksVsQuinineConcentration.pdf', transparent=True)
...
if True:
    ...
    plt.savefig('FigurePdfs/PFC_SingleAnimalTrials.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    print('Statistics for Quinine Dose Alone, ANOVA, parametric')
    print('ANOVA p-value =', stats.f_oneway(y[1, :], y[2, :], y[3, :], y[4, :]))

    # linear regression
    x_linregress = []
    for i in range(5):
        x_linregress.append(quinine_concentrations[i] * np.ones(shape=y[i, :].shape))
    x_linregress = np.concatenate(x_linregress, axis=0)
    y_linregress = np.concatenate([y[0, :], y[1, :], y[2, :], y[3, :], y[4, :]], axis=0)
    print('Linear Regression', stats.linregress(x_linregress, y_linregress))


    print('PARAMETRIC pairwise comparisons:')
    pvalues, pairs = DataProcessingFunctions.multi_pairwise_t_test_bonferroni_corrected(np.transpose(integrated_DA, [1, 0]))
    Qs = ['W', 'Q1', 'Q2', 'Q3', 'Q4']
    for i in range(len(pvalues)):
        print('Pairwise t-test', Qs[pairs[i][0]], 'vs.', Qs[pairs[i][1]], 'pvalue =', pvalues[i])

    plt.savefig('FigurePdfs/PFC_FVsQuinineConcentrationSummary.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    plt.savefig('FigurePdfs/PFC_AverageResponseVsQuinineConcentration.pdf', transparent=True)
...
if True:
    ...
    plt.savefig('FigurePdfs/PFC_ReleaseVsBlockNumber.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    for i in range(5):
        ...
        axis.set_ylim([-0.25, 1.75])
    plt.savefig('FigurePdfs/PFC_PerieventAverage_BeamBreak.pdf', transparent=True)
