...
data_directory_path = '../../OriginalData/CarouselExperimentsData/Sucrose/NAcCore/'  # directory for data files
perievent_window = [10, 20]
baseline_window = [-10, -5]                           # window for baselining the signal
...
if True:
    DataProcessingFunctions.plot_dataset(data[3], xlim=[290, 690], ylim=[-2.5, 8], solutionChar='S')
    plt.savefig('FigurePdfs/SUCROSE_NAc_ExampleDataset.pdf', transparent=True)
    plt.show()
...
if False:
    ...
    np.save('SUCROSE_NAc_lick_counts', lick_counts)
    ...
    plt.savefig('FigurePdfs/SUCROSE_NAc_NumberOfLicksVsQuinineConcentration.pdf', transparent=True)
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_NAc_SingleAnimalTrials.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    #integrated_DA_NAcCore = np.load('NAc_DA_vs_Quinine_SummaryData.npy')
    ...
    print('Statistics for Quinine summary plot (NAcCore)')
    ...
    plt.savefig('FigurePdfs/SUCROSE_NAc_FVsQuinineConcentrationSummary.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    for i in range(5):
        ...
        axis.set_ylim([-1, 1.5])
    plt.savefig('FigurePdfs/SUCROSE_NAc_AverageResponseVsQuinineConcentration.pdf', transparent=True)
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_NAc_ReleaseVsBlockNumber.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    plt.savefig('FigurePdfs/SUCROSE_NAc_PerieventAverage_BeamBreak.pdf', transparent=True)
