...
# Produces the figures for analysis of the isosbestic signal for the sucrose, quinine, and
# quinine + sucrose experiments.
...
def analyze_dataset(
    ...
    metadata, window_ts, windows = DataProcessingFunctions.get_all_perievent_window_data(experiment_dictionary['data'], perievent_window, zscore=zscore_data, plot_isosbestic=True)
    ...
    if True:
        ...
        plt.ylim([-1,1])
        plt.savefig('FigurePdfs/IsosbesticFigures/' + header + 'F_vs_ConcentrationSummary.pdf', transparent=True)
        plt.show()
    ...
    #         axis.plot(window_ts[I], multiplicative_factor * m[I], color='k')
    #
    #         if i == 0:
