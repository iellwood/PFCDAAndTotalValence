...
data_directory_path = '../../OriginalData/CarouselExperimentsData/SucroseAndQuinine/NAcCore/'  # directory for data files
PFC_baseline_window = [-10, -5]                           # window for baselining the signal
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_ExampleDataset.pdf', transparent=True)
    plt.show()
...
if False:
    ...
    np.save('SUCROSE_AND_QUININE_NAc_lick_counts', lick_counts)
    ...
    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_NumberOfLicksVsQuinineConcentration.pdf', transparent=True)
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_SingleAnimalTrials.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    for animal_number in range(number_of_animals):
        plt.plot(quinine_concentrations, integrated_DA[animal_number, :], color=[0.75, 0.75, 0.75], marker='o')
    ...
    np.save('SUCROSE_AND_QUININE_NAc_DA_vs_Quinine_SummaryData', integrated_DA)

    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_FVsQuinineConcentrationSummary.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    for i in range(5):
        axis = axes[i]
        m = np.mean(means[:, i, :], 0)
        m = m - np.mean(m[np.logical_and(window_ts <= PFC_baseline_window[1], window_ts >= PFC_baseline_window[0])])
        sem = np.std(means[:, i, :], 0)/np.sqrt(means[:, i, :].shape[0])

        # Uncomment to see every animal
        #axis.plot(window_ts, np.transpose(means[:, i, :], [1, 0]), color=[0.8, 0.8, 0.8])
    ...
    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_AverageResponseVsQuinineConcentration.pdf', transparent=True)
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_ReleaseVsBlockNumber.pdf', transparent=True)
    plt.show()
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_AND_QUININE_NAc_PerieventAverage_BeamBreak.pdf', transparent=True)
