...
sucrose_concentrations = [0.0, 29.214, 29.214*2, 29.214*4, 29.214*8]      # concentrations of sucrose in mM
data_directory_path = '../../OriginalData/CarouselExperimentsData/Sucrose/mPFC/'  # directory for data files
PFC_baseline_window = [-1, 0]                           # window for baselining the signal
averaging_window = [0, 5]
perievent_window = [10, 20]

# These indices are used in the metadata associated with the peri-event windows
ANIMAL_NUMBER = 0
TRIAL_NUMBER = 1
PORT = 2
SUCROSE_TRIAL_NUMBER = 3
...
if True:
    DataProcessingFunctions.plot_dataset(data[0], xlim=[290, 690], ylim=[-2.5, 12], solutionChar='S', zscore=False)
    plt.savefig('FigurePdfs/SUCROSE_PFC_ExampleDataset.pdf', transparent=True)
    plt.show()

# Collects all the windows from all the animals. The metadata tells the animal number and port for everyone window
metadata, window_ts, windows = DataProcessingFunctions.get_all_perievent_window_data(data, perievent_window, zscore=True)

# FIGURE PANEL: plot the lick rate as a function of sucrose concentration
if True:
    lick_counts = []
    ...
    np.save('SUCROSE_PFC_lick_counts', lick_counts)
    fig = plt.figure(figsize=(2, 4))
    plt.subplots_adjust(left=0.25, bottom=0.25)


    plt.errorbar(sucrose_concentrations, np.mean(lick_counts, 1), yerr=np.std(lick_counts, 1)/np.sqrt(lick_counts.shape[1]), capsize=5)
    prettyplot.no_box()
    #plt.ylim([1, 6])
    plt.xticks(sucrose_concentrations)
    prettyplot.ylabel('number of licks')
    prettyplot.xlabel('sucrose concentration mM')
    plt.savefig('FigurePdfs/SUCROSE_PFC_NumberOfLicksVsSucroseConcentration.pdf', transparent=True)

    plt.show()

#FIGURE PANEL: Plots the latencies for each port.
# Note that we have not included water as after every water trial there is a 50% or 100% of a sucrose trial for the first/second water trial
# In contrast, for sucrose trials, they are always followed by water trials. Thus we cannot compare these latencies.
# if False:
#     fig = plt.figure(figsize=(2, 4))
#     latencies = DataProcessingFunctions.get_latencies(metadata)
#     plt.subplots_adjust(left=0.25, bottom=0.25)
#     plt.errorbar(sucrose_concentrations, np.mean(latencies, 1), yerr=(np.std(latencies, 1)/np.sqrt(latencies.shape[1])), capsize=5)
#     prettyplot.no_box()
#     plt.ylim([0, 40])
#     plt.xticks(sucrose_concentrations)
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_PFC_SingleAnimalTrials.pdf', transparent=True)
    plt.show()
...
# FIGURE PANEL: Plots the size of the dopamine response vs. the concentration of sucrose
if True:
    fig = plt.figure(figsize=(3, 4))
    fig.add_axes([0.2, 0.2, .6, .7])

    for animal_number in range(number_of_animals):
        plt.scatter(sucrose_concentrations, integrated_DA[animal_number, :], color=[0.75, 0.75, 0.75], marker='o')

    plt.errorbar(sucrose_concentrations, np.mean(integrated_DA, 0),
    ...
    plt.xticks(sucrose_concentrations)
    prettyplot.no_box()
    prettyplot.xlabel("Sucrose conc. mM")
    ...
    print('Statistics for Sucrose summary plot (mPFC)')
    ...
    plt.savefig('FigurePdfs/SUCROSE_PFC_FVsSucroseConcentrationSummary.pdf', transparent=True)
    plt.show()
...
if True:
    ...
    for i in range(5):
        axis = axes[i]
        m = np.mean(means[:, i, :], 0)
        m = m - np.mean(m[np.logical_and(window_ts <= 0, window_ts >= -8)])
    ...
    plt.savefig('FigurePdfs/SUCROSE_PFC_AverageResponseVsSucroseConcentration.pdf', transparent=True)
    ...
    for block_number in range(number_of_blocks):
        ...
        for animal_number in range(number_of_animals):
            ...
            if np.sum(Z) >= block_size * number_of_blocks: # Test if there are enough blocks
                ...
                I = np.logical_and(block_number * block_size <= metadata[:, SUCROSE_TRIAL_NUMBER], (block_number + 1) * block_size > metadata[:, SUCROSE_TRIAL_NUMBER])
                if port is not None:
...
# SUPP. FIGURE PANEL: Plots dopamine vs. time across all animals in blocks of 4 trials (to see time dependence of the sucrose response)
if False:
    block_size = 4
    number_of_blocks = 6
    ...
    plt.savefig('FigurePdfs/SUCROSE_PFC_ReleaseVsBlockNumber.pdf', transparent=True)
    plt.show()
...
if False:
    ...
    plt.savefig('FigurePdfs/SUCROSE_PFC_PerieventAverage_BeamBreak.pdf', transparent=True)
