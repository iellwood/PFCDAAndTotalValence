...
# Produces the non-heatmap plots of from the fluorescence and tracking data of mice exploring the EPM
...
def perform_analysis_for_experiment(experiment):
    ...
    def plot_EPM_Example(dataset, filename):

        f = dataset['dF/F Excitation after isosbestic subtraction']
        x = dataset['x_coordinate']
        y = dataset['y_coordinate']


        f_sorted = np.sort(f)
        n = f_sorted.shape[0]
        vmin = f_sorted[n//64]
        vmax = f_sorted[n - n//64]
        f = f - vmin
        f = f/(vmax - vmin)
        ...
        for i in range(len(f) - 1):
            ...
            colors.append(plasma(0.5 * (f[i] + f[i + 1])))
    ...
    plot_EPM_Example(dataset, 'Dataset 1')
    ...
    plt.savefig('FigurePdfs/' + condition + '_EPMSingleAnimalExample.jpg', transparent=True, dpi=300)
    plt.show()
    ...
    def get_zone_means(d, center, center_radius, arm_length):
        ...
        x = d['x_coordinate']
        y = d['y_coordinate']
        f = d['dF/F Excitation after isosbestic subtraction']
    ...
    joint_means = np.concatenate([familiar_closed_means[:, None], novel_closed_means[:, None], center_means[:, None], open_means[:, None]], 1)
    joint_means = np.transpose(joint_means)

    p, F = statsfunctions.repeated_measures_ANOVA(np.arange(4), joint_means)
    print(condition + '; Repeated Measures ANOVA: F =', F, 'p value =', p)
