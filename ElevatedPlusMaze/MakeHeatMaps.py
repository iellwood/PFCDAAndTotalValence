...
# Produces the heatmaps from the tracking and fluorescence data. The computed heatmaps are stored in HeatmapData
...
def make_heatmap_for_condition(experiment, image_half_width):
    ...
    for i in range(len(dataset)):
        ...
        image, mask = make_heatmap(dataset[i], center, arm_length*1.1, image_half_width, arm_width/10, arm_width/5)
        images.append(image)
        masks.append(mask)
    ...
    np.savez('HeatmapData/' + condition +'_heatmapdata', images=images, masks=masks, average_width=arm_width, center=center, arm_length=arm_length, center_radius=center_radius)
